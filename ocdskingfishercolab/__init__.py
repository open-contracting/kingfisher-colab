...
def calculate_coverage(fields, scope=None, sql=True):
    """
    Calculates the coverage of one or more fields using the summary tables produced by Kingfisher Summarize's `--field-lists` option. Returns the coverage of each field and the co-occurrence coverage of all the fields.
    ...
    `fields` is a list of fields to measure the coverage of, specified using JSON Pointer relative to the object summarized in the scope table; e.g. to measure the coverage of `/awards/value/amount` against `awards_summary`, set `scope` to `awards` and `fields` to `value/amount`.
    
    To specify fields that are not children of the scope table, use the JSON Pointer relative to the release, prepended with `/`; e.g. `/tender/procurementMethod`.
    
   For arrays, a field is counted if it appears in **any** object in the array, e.g. if `scope` is set to `awards` and `field` is set to `items/description`, at least one item must have a description for the coverage to be non-zero.
   
   To specify that a field must appear in **all** objects in the array, prepend the field with `ALL `, e.g. if `scope` is set to `awards` and `field` is set to `ALL items/description`, all items must have a description for the coverage to be non-zero.
    

    To measure the co-occurence of fields across awards and contracts related by `/contracts/awardID`, set `scope` to `awards` and prepend `contracts/` to the path of fields in contracts, or set `scope` to `contracts` and prepend `awards/` to the path of fields in awards; e.g. to measure how many contracts have a value and a related award with suppliers, set `scope` to `contracts` and `fields` to `['value', 'awards/suppliers/id']`.
    ...
    *on the same release/record* has a value you can specify `/awards/value`.
    ...
    :param str scope: a table name as described above; defaults to the parent table of the first item in the fields list.
